        ...
        stage('Prepare Environment Config') {
            steps {
                script {
                    ...
                    echo "Looking for credentials with prefix: ${env.CRED_PREFIX}"
                    ...
                    def expectedCreds = [
                        ...
                        "${env.CRED_PREFIX}-twilio-service-sid"
                    ]
                    echo "Expected credentials: ${expectedCreds.join(', ')}"
                    ...
                    try {
                        withCredentials([
                            ...
                            echo "Inside withCredentials block - credentials loaded successfully"
                            ...
                            echo "Adding ${secrets.size()} secrets to environment file"

                            secrets.each { key, value ->
                                envLines += "${key}=${value}\n"
                            }

                            echo "Writing environment file to ${DEPLOY_DIR}/env.conf"
                        ...
                        echo "Exited withCredentials block successfully"
                    } catch (Exception e) {
                        error "Failed to prepare environment config: ${e.message}\n" +
                              "Please verify these credentials exist in Jenkins:\n${expectedCreds.join('\n')}"
                    }
                }
            }
        }
